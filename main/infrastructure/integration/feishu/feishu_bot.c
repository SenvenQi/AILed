...
{
    const size_t out_cap = 2048;
    uint8_t *out = heap_caps_malloc(out_cap, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!out) return -1;
    size_t pos = 0;
    if (!pb_write_tag(out, out_cap, &pos, 1, 0) || !pb_write_varint(out, out_cap, &pos, f->seq_id)) { free(out); return -1; }
    if (!pb_write_tag(out, out_cap, &pos, 2, 0) || !pb_write_varint(out, out_cap, &pos, f->log_id)) { free(out); return -1; }
    if (!pb_write_tag(out, out_cap, &pos, 3, 0) || !pb_write_varint(out, out_cap, &pos, (uint32_t)f->service)) { free(out); return -1; }
    if (!pb_write_tag(out, out_cap, &pos, 4, 0) || !pb_write_varint(out, out_cap, &pos, (uint32_t)f->method)) { free(out); return -1; }

    for (size_t i = 0; i < f->header_count; i++) {
        uint8_t hb[256];
        size_t hlen = 0;
        if (!ws_encode_header(hb, sizeof(hb), &hlen, f->headers[i].key, f->headers[i].value)) { free(out); return -1; }
        if (!pb_write_bytes(out, out_cap, &pos, 5, hb, hlen)) { free(out); return -1; }
    }
    if (payload && payload_len > 0) {
        if (!pb_write_bytes(out, out_cap, &pos, 8, payload, payload_len)) { free(out); return -1; }
    }
    int ret = esp_websocket_client_send_bin(s_ws_client, (const char *)out, pos, timeout_ms);
    free(out);
    return ret;
}
...
static char *feishu_api_call(const char *url, const char *method, const char *post_data)
{
    ...
    if (!auth_header) { esp_http_client_cleanup(client); free(resp.buf); return NULL; }
    snprintf(auth_header, 600, "Bearer %s", s_tenant_token);
...
static esp_err_t feishu_pull_ws_config(void)
{
    ...
    strncpy(s_ws_url, url_j->valuestring, sizeof(s_ws_url) - 1);
    char sid[24] = {0};
...
static void feishu_ws_task(void *arg)
{
    ...
    while (1) {
        ...
        esp_websocket_client_config_t ws_cfg = {
            .uri = s_ws_url,
            .buffer_size = 2048,
            .task_stack = FEISHU_WS_INNER_STACK,
...
static void handle_message_event(cJSON *event)
{
    ...
    ESP_LOGI(TAG, "Message from %s in %s(%s): %.60s%s", sender_id, chat_id, chat_type, cleaned, strlen(cleaned) > 60 ? "..." : "");

    cJSON *bus_msg = cJSON_CreateObject();
    cJSON_AddStringToObject(bus_msg, "text", cleaned);
    cJSON_AddStringToObject(bus_msg, "chat_id", chat_id);
    if (message_id[0]) {
        cJSON_AddStringToObject(bus_msg, "message_id", message_id);
    }
    cJSON_AddStringToObject(bus_msg, "channel", "feishu");
    char *bus_str = cJSON_PrintUnformatted(bus_msg);
    cJSON_Delete(bus_msg);
    if (bus_str) {
        msg_bus_publish(MSG_TYPE_USER_INPUT, bus_str, 1000);
        free(bus_str);
    }
...
esp_err_t feishu_bot_init(void)
{
    ...
    if (nvs_open(FEISHU_NVS_NAMESPACE, NVS_READONLY, &nvs) == ESP_OK) {
        char tmp_id[64] = {0};
        char tmp_secret[128] = {0};
        size_t len_id = sizeof(tmp_id);
        size_t len_secret = sizeof(tmp_secret);

        if (nvs_get_str(nvs, FEISHU_NVS_KEY_APP_ID, tmp_id, &len_id) == ESP_OK &&
            nvs_get_str(nvs, FEISHU_NVS_KEY_APP_SEC, tmp_secret, &len_secret) == ESP_OK &&
            tmp_id[0] && tmp_secret[0]) {
    ...
    if (!loaded) {
        strncpy(s_app_id, FEISHU_DEFAULT_APP_ID, sizeof(s_app_id) - 1);
        strncpy(s_app_secret, FEISHU_DEFAULT_APP_SEC, sizeof(s_app_secret) - 1);
        feishu_set_credentials(s_app_id, s_app_secret);
        ESP_LOGI(TAG, "Feishu default credentials saved to NVS");
    }

    ESP_LOGI(TAG, "Feishu credentials ready (app_id=%.8s...)", s_app_id);
    return ESP_OK;
}
        ...
        if (json_str) {
            ...
            if (resp) {
                ...
                if (root) {
                    ...
                    if (code_j && code_j->valueint != 0) {
                        cJSON *msg = cJSON_GetObjectItem(root, "msg");
                        ESP_LOGW(TAG, "Send failed: code=%d, msg=%s", code_j->valueint, msg ? msg->valuestring : "unknown");
                        all_ok = 0;
                    } else {
    ...
    if (resp) {
        ...
        if (root) {
            ...
            if (code_j && code_j->valueint == 0) {
                ...
                cJSON *msg = cJSON_GetObjectItem(root, "msg");
                ESP_LOGW(TAG, "Reply failed: code=%d, msg=%s", code_j ? code_j->valueint : -1, msg ? msg->valuestring : "unknown");
            }
            cJSON_Delete(root);
        }
        free(resp);
...
{
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(FEISHU_NVS_NAMESPACE, NVS_READWRITE, &nvs);
    if (err != ESP_OK) return err;

    nvs_set_str(nvs, FEISHU_NVS_KEY_APP_ID, app_id);
    nvs_set_str(nvs, FEISHU_NVS_KEY_APP_SEC, app_secret);
    nvs_commit(nvs);
    nvs_close(nvs);
